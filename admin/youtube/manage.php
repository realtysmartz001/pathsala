...
if (isset($_GET['toggle'])) {
    $id = (int) $_GET['toggle'];
    $conn->query("UPDATE youtube_videos SET published = 1 - published WHERE id = $id");
...
if (isset($_GET['move']) && isset($_GET['dir'])) {
    ...
    $stmt = $conn->prepare("SELECT id, display_order FROM youtube_videos WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    ...
    if ($current) {
        if ($dir === 'up') {
            $neighbourQ = $conn->prepare("SELECT id, display_order FROM youtube_videos WHERE display_order < ? ORDER BY display_order DESC LIMIT 1");
        } else {
            $neighbourQ = $conn->prepare("SELECT id, display_order FROM youtube_videos WHERE display_order > ? ORDER BY display_order ASC LIMIT 1");
        }
        ...
        if ($neighbour) {
            $swap = $conn->prepare("UPDATE youtube_videos SET display_order = ? WHERE id = ?");
...
$result = $conn->query("SELECT id, title, youtube_id, thumbnail_url, location, display_order, published
                         FROM youtube_videos
...
<head>
  ...
  <style>
    ...
    .thumb { width:96px; height:54px; object-fit:cover; border-radius:6px; border:1px solid #383838; }
  </style>
</head>
<body>
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h4><i class="bi bi-youtube text-danger"></i> Manage YouTube Videos</h4>
    <a href="add.php" class="btn btn-red"><i class="bi bi-plus-circle"></i> Add Video</a>
  </div>

  <?php if (isset($_GET['msg'])): ?>
    <div class="alert alert-success">
      <?= $_GET['msg'] === 'added' ? 'Video added successfully.' : 'Video deleted successfully.' ?>
    </div>
  <?php endif; ?>
    ...
    <table class="table table-borderless mb-0">
      ...
      <tbody>
        <?php $i = 1; while ($row = $result->fetch_assoc()): ?>
          <tr>
            <td><?= $i++ ?></td>
            <td><img class="thumb" src="<?= htmlspecialchars($row['thumbnail_url']) ?>" alt=""></td>
            ...
            <td>
              <a class="icon-btn" href="https://www.youtube.com/watch?v=<?= urlencode($row['youtube_id']) ?>" target="_blank" title="View on YouTube"><i class="bi bi-box-arrow-up-right"></i></a>
              ...
              <a class="icon-btn text-danger" href="manage.php?delete=<?= $row['id'] ?>" title="Delete"
                 onclick="return confirm('Delete this video?');"><i class="bi bi-trash-fill"></i></a>
            </td>
          </tr>
        <?php endwhile; ?>
      </tbody>
